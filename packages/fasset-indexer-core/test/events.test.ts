...
describe("FAsset evm events", () => {
  ...
  describe("token operations", () => {
    ...
    it("should store erc20 transfers", async () => {
      ...
      const erc20TransferEntity = await em.findOneOrFail(Entities.EvmLog, { index: erc20Transfer.index },
      ...
      const erc20TransferStored = await em.findOneOrFail(Entities.ERC20Transfer,
      ...
      const tokenBalance = await em.findOneOrFail(Entities.TokenBalance,
      ...
      const tokenBalanceReceiver = await em.findOneOrFail(Entities.TokenBalance,
  ...
  describe("system operations", () => {
    ...
    it("should store current underlying block updated event", async () => {
      ...
      const currentUnderlyingBlockUpdated = await em.findOneOrFail(Entities.CurrentUnderlyingBlockUpdated,
    ...
    it("should store core vault manager settings updated", async () => {
      ...
      const su = await em.findOneOrFail(Entities.CoreVaultManagerSettingsUpdated,
      ...
      const cvms = await em.findOneOrFail(Entities.CoreVaultManagerSettings, { fasset: FAssetType.FXRP })
  ...
  describe("agent operations", () => {
    ...
    it("should store agent created event", async () => {
      ...
      const collateralTypeAdded = await em.findOneOrFail(Entities.CollateralTypeAdded,
      ...
      const agentVaultCreated = await em.findOneOrFail(Entities.AgentVaultCreated,
      ...
      const agentVaultSettings = await em.findOneOrFail(Entities.AgentVaultSettings, { agentVault: agentVaultCreated.agentVault },
    ...
    it("should store agent vault destroy announced", async () => {
      ...
      const announceDestroy = await em.findOneOrFail(Entities.AgentDestroyAnnounced,
    ...
    it("should store agent ping", async () => {
      ...
      const agentPing = await em.findOneOrFail(Entities.AgentPing,
    ...
    it("should store agent ping response", async () => {
      ...
      const agentPingResponse = await em.findOneOrFail(Entities.AgentPingResponse,
  ...
  describe("mintings", () => {
    ...
    it("should store all minting events", async () => {
      ...
      const mintingExecuted = await em.findOneOrFail(Entities.MintingExecuted,
      ...
      const mintingPaymentDefault = await em.findOneOrFail(Entities.MintingPaymentDefault,
      ...
      const collateralReservationDeleted = await em.findOneOrFail(Entities.CollateralReservationDeleted,
    ...
    it("should store self-mint event", async () => {
      ...
      const selfMint = await em.findOneOrFail(Entities.SelfMint,
  ...
  describe("redemptions", () => {
    ...
    it("should store all redemption events", async () => {
      ...
      const redemptionPerformed = await em.findOneOrFail(Entities.RedemptionPerformed,
      ...
      const redemptionDefault = await em.findOneOrFail(Entities.RedemptionDefault,
      ...
      const redemptionRejected = await em.findOneOrFail(Entities.RedemptionRejected,
      ...
      const redemptionPaymentBlocked = await em.findOneOrFail(Entities.RedemptionPaymentBlocked,
      ...
      const redemptionPaymentFailed = await em.findOneOrFail(Entities.RedemptionPaymentFailed,
    ...
    it("should store redeemed in collateral event", async () => {
      ...
      const redeemedInCollateral = await em.findOneOrFail(Entities.RedeemedInCollateral,
    ...
    it("should store redemption request resolutions", async () => {
      ...
      const redemptionRequestedEvent = await fixture.generateEvent(EVENTS.ASSET_MANAGER.REDEMPTION_REQUESTED, assetManager)
      await storer.processEventUnsafe(em, redemptionRequestedEvent)
      const redemptionRequestedBefore = await em.findOneOrFail(Entities.RedemptionRequested,
        { evmLog: { index: redemptionRequestedEvent.index, block: { index: redemptionRequestedEvent.block.index } } })
      expect(redemptionRequestedBefore.resolution).to.equal(RedemptionResolution.NONE)
      const redemptionPerformedEvent = await fixture.generateEvent(EVENTS.ASSET_MANAGER.REDEMPTION_PERFORMED, assetManager)
      await storer.processEventUnsafe(em, redemptionPerformedEvent)
  ...
  describe("redemption tickets", () => {
    ...
    it("should store the redemption ticket created event and state", async () => {
      ...
      const redemptionTicketCreated = await em.findOneOrFail(Entities.RedemptionTicketCreated,
      ...
      const redemptionTicket = await em.findOneOrFail(Entities.RedemptionTicket, { redemptionTicketCreated }, { populate: ['redemptionTicketCreated'] })
    ...
    it("should store the redemption ticket updated event and state", async () => {
      ...
      const redemptionTicketUpdated = await em.findOneOrFail(Entities.RedemptionTicketUpdated,
      ...
      // redemption ticket
      const redemptionTicket = await em.findOneOrFail(Entities.RedemptionTicket, {
    ...
    it("should store the redemption ticket deleted event and state", async () => {
      ...
      const redemptionTicketDeleted = await em.findOneOrFail(Entities.RedemptionTicketDeleted,
  ...
  describe("liquidations", () => {
    ...
    it("should store all liquidation started events", async () => {
      ...
      const liquidationStarted = await em.findOneOrFail(Entities.LiquidationStarted,
    ...
    it("should store all full-liquidation started events", async () => {
      ...
      const liquidationStarted = await em.findOneOrFail(Entities.FullLiquidationStarted,
    ...
    it("should store all liquidation ended events", async () => {
      ...
      await storer.processEventUnsafe(em, liquidationStartedEvent)
      const liquidationStarted = await em.findOneOrFail(Entities.LiquidationEnded,
        { evmLog: { index: liquidationStartedEvent.index, block: { index: liquidationStartedEvent.block.index } } },
        { populate: ['evmLog.block', 'agentVault.address'] })
      expect(liquidationStarted).to.exist
      expect(liquidationStarted.evmLog.index).to.equal(liquidationStartedEvent.index)
    ...
    it("should store all liquidation performed events", async () => {
      ...
      const liquidationPerformed = await em.findOneOrFail(Entities.LiquidationPerformed,
  ...
  describe("emergency pause", () => {
    ...
    it("should test storing the emergency pause triggered event with old interface", async () => {
      ...
      const em = context.orm.em.fork()
      const emergencyPauseTriggered = await em.findOneOrFail(Entities.EmergencyPauseTriggered, { fasset: FAssetType.FXRP })
  ...
  describe("core vault", () => {
    ...
    it("should store transfer to core vault data", async () => {
      ...
      const ttcvs = await em.findOneOrFail(Entities.TransferToCoreVaultStarted,
      ...
      const ttcvf = await em.findOneOrFail(Entities.TransferToCoreVaultSuccessful,
      ...
      const ttcvd = await em.findOneOrFail(Entities.TransferToCoreVaultDefaulted,
    ...
    it("should store return from core vault", async () => {
      ...
      const tfcvr = await em.findOneOrFail(Entities.ReturnFromCoreVaultRequested,
      ...
      const tfcvc = await em.findOneOrFail(Entities.ReturnFromCoreVaultConfirmed,
      ...
      const tfcvd = await em.findOneOrFail(Entities.ReturnFromCoreVaultCancelled,
    ...
    it("should store core vault redemption requested", async () => {
      ...
      const cvrr = await em.findOneOrFail(Entities.CoreVaultRedemptionRequested,
  ...
  describe("collateral pool", () => {
    ...
    it("should store collateral pool cp-entered event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpe = await em.findOneOrFail(Entities.CPEntered,
    ...
    it("should store collateral pool cp-exited event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpe = await em.findOneOrFail(Entities.CPExited,
        { evmLog: { block: { index: ecpe.block.index }, index: ecpe.index } },
        { populate: ['tokenHolder'] }
      )
      expect(cpe.fasset).to.equal(FAssetType.FXRP)
    ...
    it("should store collateral pool cp-claimed-reward event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const ccr = await em.findOneOrFail(Entities.CPClaimedReward,
    ...
    it("should store collateral pool cp-fee-debt-paid event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpcfdp = await em.findOneOrFail(Entities.CPFeeDebtPaid,
    ...
    it("should store collateral pool cp-fee-debt-changed event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpcfdc = await em.findOneOrFail(Entities.CPFeeDebtChanged,
    ...
    it("should store collateral pool cp-fees-withdrawn event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpcfw = await em.findOneOrFail(Entities.CPFeesWithdrawn,
    ...
    it("should store collateral pool cp-paid-out event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpcpo = await em.findOneOrFail(Entities.CPPaidOut,
    ...
    it("should store collateral pool cp-self-close-exited event", async () => {
      ...
      // check
      const em = context.orm.em.fork()
      const cpcsce = await em.findOneOrFail(Entities.CPSelfCloseExited,
  ...
  describe("edge cases", () => {
    ...
    it("should be able to store collateral reserved events with same ids, from different fassets", async () => {
      ...
      const collateralReserved = await em.findOneOrFail(Entities.CollateralReserved, { fasset: FAssetType.FXRP })
      ...
      const collateralReservedBtc = await em.findOneOrFail(Entities.CollateralReserved, { fasset: FAssetType.FBTC })
    ...
    it("should not allow two events with same log index and block index", async () => {
      ...
      await storer.processEventUnsafe(em, event1)
      await storer.processEventUnsafe(em, event2)
      const logs = await em.findAll(Entities.EvmLog)
    ...
    it("should not store an event that is not processed", async () => {
      ...
      const evmLogs = await em.findAll(Entities.EvmLog)
